...
from dbt.adapters.gaussdbdws.record import GaussDBDWSRecordReplayHandle
...
logger = AdapterLogger("GaussDBDWS")


@dataclass
class GaussDBDWSCredentials(Credentials):
    host: str
    user: str
    # Annotated is used by mashumaro for jsonschema generation
    port: Annotated[Port, Minimum(0), Maximum(65535)]
    password: str  # on gaussdbdws the password is mandatory
...
class GaussDBDWSConnectionManager(SQLConnectionManager):
    TYPE = "gaussdbdws"
    ...
    def exception_handler(self, sql):
        try:
            ...
            logger.debug("GaussDBDWS error: {}".format(str(e)))
    ...
    def open(cls, connection):
        ...
        kwargs = {}
        # we don't want to pass 0 along to connect() as gaussdbdws will try to
        ...
        def connect():
            ...
            if rec_mode is not None:
                ...
                # use a wrapper.
                handle = GaussDBDWSRecordReplayHandle(handle, connection)
