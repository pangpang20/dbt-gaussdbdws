...
class TestCustomSchema(BaseTestCustomSchema):
    def test__gaussdbdws_handles__custom_schema_with_no_prefix(self, project, macros):
...
class TestCustomSchemaWithCustomMacro(BaseTestCustomSchema):
    ...
    def test__gaussdbdws_handles__custom_schema_with_custom_macro(self, project, macros):
        project.run_sql(_VALIDATION_SQL)
        run_dbt(["seed"])
        results = run_dbt(["run"])
        assert len(results) == 3
...
class TestCustomSchemaWithPrefix(BaseTestCustomSchema):
    ...
    def test__gaussdbdws__custom_schema_with_prefix(self, project, macros):
        project.run_sql(_VALIDATION_SQL)
        run_dbt(["seed"])
        results = run_dbt(["run"])
        assert len(results) == 3
...
class TestCustomSchemaWithPrefixAndDispatch(BaseTestCustomSchema):
    ...
    def test__gaussdbdws__custom_schema_with_prefix_and_dispatch(
...
class TestCustomSchemaWithCustomMacroFromModelName(BaseTestCustomSchema):
    ...
    def test__gaussdbdws__custom_schema_from_model_name(
        self, project, macros, project_config_update
    ):
        project.run_sql(_VALIDATION_SQL)
        run_dbt(["seed"])
