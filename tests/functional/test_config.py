...
from dbt.adapters.gaussdbdws import GaussDBDWSCredentials
...
class BaseConfigTest(TestCase):
    ...
    def setUp(self):
        ...
        self.default_profile_data = {
            "default": {
                "outputs": {
                    "gaussdbdws": {
                        "type": "gaussdbdws",
                        "host": "gaussdbdws-db-hostname",
                        "port": 5555,
                        "user": "db_user",
                        "pass": "db_pass",
                        "dbname": "gaussdbdws-db-name",
                        "schema": "gaussdbdws-schema",
                        "threads": 7,
                    },
                    "with-vars": {
            ...
            "other": {
                "outputs": {
                    "other-gaussdbdws": {
                        "type": "gaussdbdws",
                        "host": "other-gaussdbdws-db-hostname",
                        "port": 4444,
                        "user": "other_db_user",
                        "pass": "other_db_pass",
                        "dbname": "other-gaussdbdws-db-name",
                        "schema": "other-gaussdbdws-schema",
                        "threads": 2,
                    }
                },
                "target": "other-gaussdbdws",
            },
            "empty_profile_data": {},
        }
        ...
        self.env_override = {
            "env_value_type": "gaussdbdws",
            "env_value_host": "env-gaussdbdws-host",
            "env_value_port": "8000",
            "env_value_user": "env-gaussdbdws-user",
            "env_value_pass": "env-gaussdbdws-pass",
            "env_value_dbname": "env-gaussdbdws-dbname",
            "env_value_schema": "env-gaussdbdws-schema",
...
class TestProfile(BaseConfigTest):
    ...
    def test_from_raw_profiles(self):
        ...
        self.assertEqual(profile.target_name, "gaussdbdws")
        self.assertEqual(profile.threads, 7)
        self.assertTrue(isinstance(profile.credentials, GaussDBDWSCredentials))
        self.assertEqual(profile.credentials.type, "gaussdbdws")
        self.assertEqual(profile.credentials.host, "gaussdbdws-db-hostname")
        ...
        self.assertEqual(profile.credentials.schema, "gaussdbdws-schema")
        self.assertEqual(profile.credentials.database, "gaussdbdws-db-name")

    def test_missing_type(self):
        del self.default_profile_data["default"]["outputs"]["gaussdbdws"]["type"]
        with self.assertRaises(dbt.exceptions.DbtProfileError) as exc:
            self.from_raw_profiles()
        self.assertIn("type", str(exc.exception))
        self.assertIn("gaussdbdws", str(exc.exception))
        self.assertIn("default", str(exc.exception))

    def test_bad_type(self):
        self.default_profile_data["default"]["outputs"]["gaussdbdws"]["type"] = "invalid"
        with self.assertRaises(dbt.exceptions.DbtProfileError) as exc:
            self.from_raw_profiles()
    ...
    def test_invalid_credentials(self):
        del self.default_profile_data["default"]["outputs"]["gaussdbdws"]["host"]
        with self.assertRaises(dbt.exceptions.DbtProfileError) as exc:
            self.from_raw_profiles()
        self.assertIn("Credentials", str(exc.exception))
        self.assertIn("gaussdbdws", str(exc.exception))
    ...
    def test_missing_target(self):
        ...
        del profile["target"]
        profile["outputs"]["default"] = profile["outputs"]["gaussdbdws"]
        profile = self.from_raw_profiles()
        self.assertEqual(profile.profile_name, "default")
...
class TestProfileFile(BaseConfigTest):
    ...
    def test_profile_override(self):
        ...
        self.assertEqual(profile.target_name, "other-gaussdbdws")
        self.assertEqual(profile.threads, 3)
        self.assertTrue(isinstance(profile.credentials, GaussDBDWSCredentials))
        self.assertEqual(profile.credentials.type, "gaussdbdws")
        self.assertEqual(profile.credentials.host, "other-gaussdbdws-db-hostname")
        ...
        self.assertEqual(profile.credentials.schema, "other-gaussdbdws-schema")
        self.assertEqual(profile.credentials.database, "other-gaussdbdws-db-name")
        self.assertEqual(profile, from_raw)
    ...
    def test_env_vars(self):
        ...
        self.assertEqual(profile.credentials.type, "gaussdbdws")
        self.assertEqual(profile.credentials.host, "env-gaussdbdws-host")
        self.assertEqual(profile.credentials.port, 8000)
        self.assertEqual(profile.credentials.user, "env-gaussdbdws-user")
        self.assertEqual(profile.credentials.password, "env-gaussdbdws-pass")
        self.assertEqual(profile, from_raw)
    ...
    def test_cli_and_env_vars(self):
        ...
        self.args.vars = {"cli_value_host": "cli-gaussdbdws-host"}
        renderer = dbt.config.renderer.ProfileRenderer({"cli_value_host": "cli-gaussdbdws-host"})
        with mock.patch.dict(os.environ, self.env_override):
        ...
        self.assertEqual(profile.threads, 1)
        self.assertEqual(profile.credentials.type, "gaussdbdws")
        self.assertEqual(profile.credentials.host, "cli-gaussdbdws-host")
        self.assertEqual(profile.credentials.port, 8000)
        self.assertEqual(profile.credentials.user, "env-gaussdbdws-user")
        self.assertEqual(profile.credentials.password, "env-gaussdbdws-pass")
        self.assertEqual(profile, from_raw)
