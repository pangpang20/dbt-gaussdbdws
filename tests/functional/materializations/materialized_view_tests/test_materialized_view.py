...
class TestGaussDBDWSMaterializedViewsBasic(MaterializedViewBasic):
...
class GaussDBDWSMaterializedViewChanges(MaterializedViewChanges):
    @pytest.fixture(scope="class", autouse=True)
    def models(self):
        yield {
            "my_table.sql": MY_TABLE,
    ...
    def change_config_via_replace(project, materialized_view):
        # dbt-gaussdbdws does not currently monitor changes of this type
        pass


class TestGaussDBDWSMaterializedViewChangesApply(
    GaussDBDWSMaterializedViewChanges, MaterializedViewChangesApplyMixin
):
    @pytest.mark.skip("dbt-gaussdbdws does not currently monitor replace changes.")
...
class TestGaussDBDWSMaterializedViewChangesContinue(
    GaussDBDWSMaterializedViewChanges, MaterializedViewChangesContinueMixin
):
    @pytest.mark.skip("dbt-gaussdbdws does not currently monitor replace changes.")
...
class TestGaussDBDWSMaterializedViewChangesFail(
    GaussDBDWSMaterializedViewChanges, MaterializedViewChangesFailMixin
):
    @pytest.mark.skip("dbt-gaussdbdws does not currently monitor replace changes.")
    def test_change_is_not_applied_via_replace(self, project, my_materialized_view):
