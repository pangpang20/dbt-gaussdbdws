...
from dbt.adapters.gaussdbdws import Plugin as GaussDBDWSPlugin, GaussDBDWSAdapter
...
class TestGaussDBDWSConnection(TestCase):
    def setUp(self):
        self.target_dict = {
            "type": "gaussdbdws",
            "dbname": "gaussdb",
        ...
        self.patcher = mock.patch("dbt.adapters.gaussdbdws.connections.psycopg2")
        ...
        self.adapter = GaussDBDWSAdapter(self.config, self.mp_context)
        ...
        inject_adapter(self.adapter, GaussDBDWSPlugin)
    ...
    def test_quoting_on_drop(self):
        ...
        self.mock_execute.assert_has_calls(
            [
                mock.call(
                    '/* dbt */\ndrop table if exists "gaussdb"."test_schema".test_table cascade',
                    None,
                )
            ]
        )
    ...
    def test_quoting_on_truncate(self):
        ...
        self.mock_execute.assert_has_calls(
            [mock.call('/* dbt */\ntruncate table "gaussdb"."test_schema".test_table', None)]
        )
    ...
    def test_quoting_on_rename(self):
        ...
        self.mock_execute.assert_has_calls(
            [
                mock.call(
                    '/* dbt */\nalter table "gaussdb"."test_schema".table_a rename to table_b',
                    None,
                )
            ]
        )
    ...
    def test_dbname_verification_is_case_insensitive(self):
        ...
        self.target_dict["dbname"] = "gaussdb"
        profile_cfg = {
            "outputs": {
                "test": self.target_dict,
            },
        ...
        self.mp_context = get_context("spawn")
        self.adapter.cleanup_connections()
        self._adapter = GaussDBDWSAdapter(self.config, self.mp_context)
        self.adapter.verify_database("gaussdb")
