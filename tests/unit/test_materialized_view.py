...
from dbt.adapters.gaussdbdws.relation_configs import GaussDBDWSIndexConfig
...
def test_index_config_changes():
    ...
    existing_indexes = frozenset(
        GaussDBDWSIndexConfig.from_dict(index) for index in [index_0_old, index_1_old, index_2_old]
    )
    ...
    new_indexes = frozenset(
        GaussDBDWSIndexConfig.from_dict(index) for index in [index_0_new, index_2_new, index_3_new]
    )
